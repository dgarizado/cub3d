...
/*   Updated: 2023/11/20 16:10:56 by vcereced         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	double radio = 12; radio/=data->size;//harcoded
	for (int angulo = 0; angulo <= 360; angulo= angulo + 20) {
    	...
    	if(data->arr[(circle_y * 10) + circle_x] == 1 || data->arr[(circle_y * 10) + circle_x] == 9)//HARCODED!!! BONUS!!!!!
			return 1;
	}
	return 0;
}
...
void ft_move_steven(double *tempx, double *tempy, double *ptr_x, double *ptr_y, t_data *data)
{
	...
	*tempy = data->steven_y + dy * DIST_FACTOR_STEVEN;
	drawLine((data->steven_x *SCALE_X) + dx*SCALE_X, (data->steven_y *SCALE_Y) + dy*SCALE_Y, (data->steven_x *SCALE_X), (data->steven_y *SCALE_Y), 0x00FF40FF, data->map, data);
...
void ft_check_door(t_data *data)
{
	...
	{
		x = cos(ft_degre_to_radian(data->angle)) * 50/data->size;		//lHARCODED  LONGITUD RAYO ROJO MINIMAP
		y = sin(ft_degre_to_radian(data->angle)) * 50/data->size;		//HARCODED  LONGITUD RAYO ROJO MINIMAP
		if(data->arr[(int)(data->py + y)*10 + (int)(data->px + x)] == 9 )//HARCODED 10 LARGO MAPA!!
			data->arr[(int)(data->py + y)*10 + (int)(data->px + x)] = 8;
	}
}
