...
/*   Updated: 2023/11/27 19:10:04 by dgarizad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	drawline_game(double x1, double y1, double y2, t_data *data)
{
	...
	{
		if (x1 >= 0 && x1 <= data->mlx->width && y1 >= 0 && y1 <= data->mlx->height)
...
void	draw_colision(int rayCount, float a, int n, t_data *data)
{
	...
	data->cast.bonus.distance[n] = fix_fish_eye(data->cast.bonus.distance[n], a,
			data->angle);
	wallheight = floor(data->mlx->height / 2) / data->cast.bonus.distance[n];
	if (last_colision_arr(data->cast.bonus.type_wall_bonus) == n)
	{
		data->map.flag = 0;
		drawline_game(rayCount, 0, (data->mlx->height / 2) - wallheight + data->up, data);
		data->map.flag = 1;
		drawline_game(rayCount, (data->mlx->height / 2) + wallheight - data->up, data->mlx->height,
			data);
	}
	data->cast.bonus.n = n;
	drawlinetexture_bonus(rayCount, (data->mlx->height / 2) - wallheight - data->up,
		(data->mlx->height / 2) + wallheight - data->up, data);
}
